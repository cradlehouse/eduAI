...
export type AdminOrg = { id: string; name: string; slug: string; content_tier: "M" | "A"; has_minors: boolean; role: MemberRole };
...
export async function getAdminOrg(): Promise<AdminOrg | null> {
  ...
  const { data } = await supabase
    .from("memberships")
    .select("role, orgs(id, name, slug, content_tier, has_minors)")
    .in("role", ["admin", "owner"])
    .order("created_at")
    .limit(1)
    .maybeSingle();

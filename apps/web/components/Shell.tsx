...
// Global header (breadcrumb = scope switcher) + a sidebar: the user block with their tokens on top,
          ...
          <div className="card mb-3 p-3">
            <details className="relative">
              <summary className="flex cursor-pointer list-none items-center gap-2">
                <span className="display flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ink text-xs text-paper">{(name[0] ?? "?").toUpperCase()}</span>
                <span className="min-w-0 flex-1 leading-tight">
                  <span className="block truncate text-sm font-medium">{name}</span>
                  <span className="block truncate text-[11px] text-muted">{role} · {nav.org?.name}</span>
                </span>
                {budget && <TokenRing spent={budget.spent} total={budget.total} />}
              </summary>
              <div className="card absolute left-0 z-20 mt-1 flex w-full flex-col p-2 text-sm">
                <div className="px-2 py-1 text-xs text-muted">{nav.email}</div>
            ...
            {budget && <div className="mt-3"><TokenBar {...budget} /></div>}
          </div>

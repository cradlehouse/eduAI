...
export function Shell({ nav, crumbs, base, sections, scope = {}, note, budget, children }: {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <header className="flex items-center gap-4 glass m-3 mb-0 rounded-[10px] px-4 py-2">
        <Link href="/" className="display text-base">Imaje</Link>
        <Crumb segs={crumbs} base={base} sections={sections} />
        <details className="relative ml-auto">
          <summary className="display flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-full bg-field text-xs text-ink">{(name[0] ?? "?").toUpperCase()}</summary>
          <div className="card absolute right-0 z-20 mt-1 flex min-w-56 flex-col p-2 text-sm">
            ...
            <Link href="/home" className="rounded-full px-2 py-1 hover:bg-field">Home</Link>
            {nav.isAdmin && <Link href="/org" className="rounded-full px-2 py-1 hover:bg-field">Organisation</Link>}
            <Link href="/account" className="rounded-full px-2 py-1 hover:bg-field">Account</Link>
            <form action={signOut}><button className="w-full rounded-full px-2 py-1 text-left hover:bg-field" type="submit">Sign out</button></form>
          </div>
        </details>
      </header>

// Money is always the same colour (money token), whichever envelope applies.
export function BudgetRing({ spent, total, scope }: { spent: number; total: number; scope: "project" | "personal" }) {
  ...
  return (
    <div className="flex items-center gap-3" aria-label={`${scope} budget: ${dollars(spent)} of ${dollars(total)} used`}>
      ...
      <div className="text-sm leading-tight">
        <div><span className="font-medium">{dollars(spent)}</span> <span className="opacity-60">/ {dollars(total)}</span></div>
        <div className="text-xs opacity-60">{scope === "project" ? "project budget" : "your budget"}</div>
      </div>
    </div>
  );
}

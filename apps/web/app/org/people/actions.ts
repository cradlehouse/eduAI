...
export async function addToProject(formData: FormData) {
  ...
  const roles = Array.from(formData.getAll("project_role")).map(String).filter(Boolean);
  if (roles.length === 0) roles.push("director");
  ...
  const { error } = await supabase.from("project_members").upsert({ org_id: org.id, project_id: projectId, user_id: userId, roles }, { onConflict: "project_id,user_id" });
  if (error) redirect(`/org/people?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/org/people");

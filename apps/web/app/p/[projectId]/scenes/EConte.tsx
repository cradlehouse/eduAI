      ...
      {cuts.map((s) => {
        ...
        return (
          <div key={s.id} className="grid grid-cols-[54px_300px_minmax(0,1fr)_96px] border-b border-line py-4">
            ...
            <div>
              <Link href={href} className={`relative block aspect-video overflow-hidden rounded-[12px] ${pic ? "bg-ink" : "border border-dashed border-line bg-paper"}`}>
                {pic ? (pic.kind === "video"
                  ...
                  : <span className="absolute inset-0 flex items-center justify-center text-xs text-muted">no takes yet · open the cut to generate</span>}
              </Link>
              <div className="mt-2 flex gap-1.5">
                {LAYERS.slice(0, 3).map((l) => { const n = layerCount(s.id, l); const reused = l === "background" && !!s.plate_take_id;
                  return <span key={l} className={`flex-1 rounded-[8px] px-1.5 py-1 text-center text-[10px] ${n || reused ? "bg-sand text-ink" : "border border-dashed border-line text-muted"}`}>{l === "background" ? "BG" : l === "character" ? "Char" : "Merged"} {n || (reused ? "plate" : "—")}</span>; })}
              </div>
            ...
            <div className="flex flex-col gap-1.5 px-4 text-[13px] leading-snug">
              ...
              {!complete && <Link href={href} className="text-xs text-explore underline">intent incomplete — finish the notes first</Link>}
            </div>
            <div className="flex flex-col items-end gap-1 text-right">
              <span className="mono text-sm">{s.duration_target_s != null ? `${s.duration_target_s.toFixed(1)}s` : "—"}</span>
              {bible.length > 0 ? bible.map((b, i) => <span key={i} className="text-[11px] text-muted">{b.name}{b.requires_consent ? " ✓" : ""}</span>)
